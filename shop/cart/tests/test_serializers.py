...
class CartSerializerTestCase(TestCase):
    def test_ok(self):
        ...
        product_1 = Product.objects.create(name='Toothpaste', slug='toothpastes', category=category_1,
                                           description='Paste for tooth',
                                           price=5, stock=5, available=True)

        product_2 = Product.objects.create(name='Towel', slug='towels', category=category_2,
                                           description='Hand towel',
                                           price=2, stock=15, available=True)
        product_cart_1 = ProductCart.objects.create(cart=cart_1, product=product_1, quantity=2)
        product_cart_2 = ProductCart.objects.create(cart=cart_1, product=product_2, quantity=5)
...
class ProductCartAddDeleteSerializerTestCase(TestCase):
    def test_ok(self):
        cart_1 = Cart.objects.create()
        product_1 = Product.objects.create(name='Toothpaste', slug='toothpastes', category=None,
                                           description='Paste for tooth',
                                           price=5, stock=5, available=True)

        product_2 = Product.objects.create(name='Towel', slug='towels', category=None,
                                           description='Hand towel',
                                           price=2, stock=15, available=True)
